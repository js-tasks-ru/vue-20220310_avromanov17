  ...
  <form class="meetup-form" @submit.prevent="submit">
    <div class="meetup-form__content">
      ...
      <meetup-agenda-item-form
        v-for="(item, idx) in localMeetup.agenda"
        :key="item.id"
        :agenda-item="item"
        class="meetup-form__agenda-item"
        @update:agendaItem="updateAgendaItem($event, idx)"
...
export default {
  ...
  methods: {
    ...
    addAgendaItem() {
      ...
      const agendaItemsCount = this.localMeetup.agenda?.length;
      if (agendaItemsCount) {
        newAgendaItem.startsAt = this.localMeetup.agenda[agendaItemsCount - 1].endsAt;
      }
      this.localMeetup = {
        ...this.localMeetup,
        agenda: [...this.localMeetup.agenda, newAgendaItem],
      };
    },
    removeAgendaItem(itemIdx) {
      this.localMeetup.agenda.splice(itemIdx, 1);
    },
    updateAgendaItem(updatedItem, itemIdx) {
      this.localMeetup.agenda[itemIdx] = updatedItem;
    },
  },
};
</script>
